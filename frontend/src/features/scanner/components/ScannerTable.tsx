...
import { RefreshCw, LayoutGrid, AlertCircle, CheckCircle2, XCircle } from 'lucide-react';
...
function ValidationPanel({ pair }: { pair: any }) {
    const checks = [
        ...
        { name: "Corr ≥ 0.85", pass: pair.correlation >= 0.85, detail: Number(pair.correlation).toFixed(3) },
        ...
        { name: "Safe Zone Check", pass: pair.zone !== "danger", detail: pair.zone?.toUpperCase() || "N/A" },
        { name: "Dedup Engine", pass: true, detail: "Unique" },
        { name: "Asset Concentration", pass: true, detail: "Pass" },
    ];
    ...
    return (
        <div className="mt-2 p-4 bg-[#0c1222] border border-[#1e3a5f] rounded-lg">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                {checks.map((c, i) => (
                    <div key={i} className={cn(
                        "flex items-center gap-2 p-1.5 rounded bg-white/[0.02] border",
                        c.pass ? "border-ui-green/20" : "border-ui-red/30"
                    )}>
                        {c.pass ? <CheckCircle2 className="w-3 h-3 text-ui-green" /> : <XCircle className="w-3 h-3 text-ui-red" />}
            ...
            {pair.qualified && (
                <div className="mt-3 p-3 bg-white/[0.03] rounded border border-[#151f35] flex items-center justify-between">
                    <div className="flex gap-6">
                        <div className="flex flex-col">
                            <span className="text-[7px] text-[#5a6a82] font-bold uppercase tracking-widest">Est Hold Time</span>
                            <span className="text-[10px] font-bold text-[#c8d6e5]">{(pair.half_life * 0.5).toFixed(0)} – {Number(pair.half_life).toFixed(0)} – {(pair.half_life * 2).toFixed(0)} Days</span>
                        </div>
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                                ...
                                <span className="text-[10px] font-bold">{pair.symbol_a} $1,000</span>
                                <span className="text-[#5a6a82]">/</span>
                                <Side side={pair.zscore > 0 ? "LONG" : "SHORT"} />
                                <span className="text-[10px] font-bold">{pair.symbol_b} ${(1000 * pair.hedge_ratio).toFixed(0)}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    if (isError) return (
        <div className="p-20 flex flex-col items-center justify-center">
            <div className="bg-[#450a0a]/20 border border-ui-red/30 p-12 rounded-lg flex flex-col items-center max-w-2xl text-center">
                ...
                <div className="text-[#c8d6e5] text-xs leading-relaxed opacity-60">API bridge (3001) is unreachable. Check Python logs.</div>
            </div>
        </div>
    );
        ...
        <div className="overflow-x-auto p-4">
            ...
            <div className="flex flex-col border-x border-[#151f35] border-b bg-[#0b1120] rounded-b-lg overflow-hidden">
                {sortedPairs.map((pair: any) => {
                    ...
                    return (
                        <div key={id} className="border-t first:border-t-0 border-[#151f35] transition-colors">
                            <div
                            ...
                            >
                                ...
                                {/* HL */}
                                <div className="text-[10px] font-bold text-[#c8d6e5] opacity-80 uppercase tabular-nums">
                                ...
                                {/* Spread */}
                                <div>
                                    <MiniSpread zscore={pair.zscore} />
                                </div>

                                {/* Zone */}
                                <div>
                                    <ZoneBadge zone={pair.zone || 'none'} sizePct={pair.sizePct} />
                                </div>
                                ...
                                <div>
                                    {isSignal ? (
                                        <button className={cn(
                                            "w-full py-1 rounded text-[9px] font-black uppercase tracking-widest text-[#06080f] shadow-lg active:scale-95 transition-all transform",
                                            pair.zscore > 0 ? "bg-ui-red" : "bg-ui-green"
                                        )}>
